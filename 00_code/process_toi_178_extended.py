...
if not save_path.exists():
    data = np.load(
        "/Users/hadden/Papers/10_chain_dynamics/03_data/TOI-178/TOI-178_maxorder3.npz"
    )
    m_inner = 1.2**(4) * 3e-6  / 0.647 #  m_p ~ R^4, stellar mass is 0.647
    masses = np.concatenate([[m_inner,], data["masses"]])
    resonances = np.concatenate(([[5,2],],data['resonances']))
    ...
    # just hardcode-- pain in the butt to find stable equilibrium with the new planet, and we only need one for now
    rvars = np.array([ 2.37793006e-03, -1.98409555e-02,  1.70487880e-02, -3.33902507e-02,
        7.94116178e-03,  1.09661337e-03, -3.14148468e+00,  9.43120535e+00,
        1.75841885e+00, -6.47544944e-01,  1.76303975e-03, -1.46988388e-02,
        1.26519803e-02, -2.50774836e-02,  3.00031652e-02, -2.27657795e-02,
       -2.08078458e-04, -2.67278914e-03, -1.01646053e-02, -3.31223302e-02])
    rc.dK2 = dK20
...
guess = eqC.copy()
dK2vals = dK20 + np.linspace(0.01,-0.005,80) 
