...
def get_planet_params():
    ...
    SELECT
        ...
        pl_refname
    FROM ps
    WHERE hostname = 'K2-138'
        AND pl_refname LIKE '%Vivien%'
    ORDER BY pl_orbper
    """
    query_default = r"""
    SELECT
    ...
    dfs = []
    for query in [query_default,query_vivien]:
    ...
    df_default,df_vivien = dfs

    # Columns you want to copy over
    cols = ["pl_orbper", "pl_orbpererr1", "pl_orbpererr2"]

    # Keep only relevant columns from default table and rename them
    df_default_sub = df_default[["pl_name"] + cols].rename(
        columns={c: f"{c}_default" for c in cols}
    )

    # Merge into Vivien table
    df_vivien_merged = df_vivien.merge(df_default_sub, on="pl_name", how="left")

    # Fill NaNs in Vivien values with default values
    for c in cols:
        df_vivien_merged[c] = df_vivien_merged[c].fillna(
            df_vivien_merged[f"{c}_default"]
        )

    # Drop helper columns if desired
    df_vivien_filled = df_vivien_merged.drop(columns=[f"{c}_default" for c in cols])

    masses = (df_vivien_filled.pl_bmasse / df_vivien_filled.st_mass) * 3e-6
    periods = df_vivien_filled.pl_orbper.values
    order = np.argsort(periods)[:-1]
    return np.array(masses[order]),np.array(periods[order])
