...
if not save_path.exists():
    ...
    df_list_preffered = df_list[1]
    print(f"Preffered posteriors: {df_list_preffered['author_name']}")
    print(f"\t mass: {df_list_preffered['mass_prior']}")
    print(f"\t eccentricity: {df_list_preffered['eccentricity_prior']}")
 
    df = df_list_preffered['samples']
    periods = np.array([df[f'period_days_{i}'].median() for i in range(df_list_preffered['nb_planets'])])
    masses = np.array([df[f'mass_planet_star_ratio_{i}'].median() for i in range(df_list_preffered['nb_planets'])])
